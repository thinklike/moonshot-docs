...
Together we'll design a product that answers a yes or no question and provides a reason.

<Frame caption="Example only. You are welcome to make a better looking version 🙂">
...
<Steps>
  ...
  <Step title="Create a multi-variant component">
    Let's create a component with three different variants: `Yes`, 'No', and 'Null'. We need a `Null` variant to use before we make our first request or when we reset the variable. These names can be upper or lower case, and you can name the variant property anything you like—for the purpose of this example we'll name it "Response".
